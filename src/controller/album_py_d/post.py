...
import src.repository.album.insert as album_insert_repository
...
def post():
    try:
        ...
        if not id_artista:
            return http_error_400('El campo `id_artista` es obligatorio.')
        ...
        album_insert_repository.register_album(
            nombre,
            descripcion,
            url_s3_foto,
            id_artista
        ...
        return http_ok_message_200('Álbum registrado correctamente.')
    except Exception:
        log_error(format_exc())
        return http_error_500('Error del servidor al intentar registrar un álbum.')
